...
jQuery("body").on( "mouseenter",".navbar-nav > .dropdown", function () {
    jQuery(this).addClass('open');
});
jQuery("body").on( "mouseleave",".navbar-nav > .dropdown", function () {
    jQuery(this).removeClass('open');
});

function toggleSection(sectionRef, callerRef) {
    var styleRef = sectionRef.prop('style');
    var displayAttr;
    var linkValue;
    if ('none' === styleRef.display) {
        displayAttr = '';
        linkValue = '\uf056';
    } else {
        displayAttr = 'none';
    ...
    styleRef.display = displayAttr;
    callerRef.attr('value', linkValue);
}

function toggleDbInfo() {
    toggleSection(jQuery('.showHideDbInfo'), jQuery('#showHideDbInfoLink'));
}
